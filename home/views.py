        ...
        if user is not None:
            ...
            if sign:
                ...
                if ser:
                    ...
                    messages.error(request, 'Either your request is pending or you are not a Member')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if user.is_staff:
                ...
                return redirect('index')  #dashboard to index page
            else:
...
from .models import City, Service, Service_Man
...
def searchProvider(request):
    ...
    if request.method == 'POST':
        ...
        # Check for valid values before querying the database
        if city_id and service_id and city_id != "" and service_id != "":
            # Fetch the matching service providers
            users = Service_Man.objects.filter(city=city_id, service=service_id)
            
            # Use get_object_or_404 to handle cases where the ID is invalid
            serv = get_object_or_404(Service, service_id=service_id)
            ct = get_object_or_404(City, id=city_id)
            ...
            # Handle the case where either city or service is not selected
            error_message = "Please select both a city and a service."
            context = {
                'cities': cities,
                'services': services,
                'page': page,
    ...
    context = {'cities': cities, 'services': services, 'page': page}
    return render(request, 'search_sp.html', context)
...
# edit booking of admin of customer
@login_required(login_url='login')
def edit_booking(request, booking_id):
    ...
    if form.is_valid():
        form.save()
        # Redirect to a valid route after saving (e.g., 'booking_list')
        return redirect('booking_list')  # Ensure 'booking_list' is a valid route name in urls.py
...
def edit_city(request, id):
    city = get_object_or_404(City, id=id)  # Get the city object
    if request.method == 'POST':
        form = CityForm(request.POST, instance=city)
        if form.is_valid():
            form.save()
            return redirect('city_list')  # Redirect after saving
    else:
        form = CityForm(instance=city)  # Prepopulate the form with current city data
    
    return render(request, 'edit_city.html', {'form': form})


# session code and handle user login
def handleLogin(request):
    page = 'login'
    if request.user.is_authenticated:
        messages.info(request, 'You are already logged in!!')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if sign:
                #  Add this block for customer session
                login(request, user)
                request.session['user_type'] = 'customer'
                request.session['customer_id'] = sign.id
                messages.success(request, 'User login successfully!')
                return redirect('index')
                ...
                if ser:
                    # ✅ Add this block for serviceman session
                    login(request, user)
                    request.session['user_type'] = 'serviceman'
                    request.session['serviceman_id'] = ser.id
                    messages.success(request, 'User login successfully!')
                    return redirect('index')
            ...
            messages.error(request, 'Username and password do not match! Please enter the correct credentials.')
            return redirect('login')
...
def dashboard_view(request):
    ...
    elif user_type == 'serviceman':
        serviceman_id = request.session.get('serviceman_id')
        # Load service-man-specific data


from django.contrib.auth import logout

def handleLogout(request):
    logout(request)  # This clears session and logs out
